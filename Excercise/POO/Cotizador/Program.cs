/*
Crear un proyecto de tipo biblioteca de clases. 
Construir tres clases dentro de un namespace llamado Billetes: Pesos, Euro y Dolar.

    Se debe lograr que los objetos de estas clases se puedan sumar, restar y comparar entre sí con total normalidad como si fueran tipos numéricos, teniendo presente que: 
    * 1 Euro equivale a 1,17 Dólares
    * 1 Dólar equivale a 102,65 Pesos.
...
Euro aux_e = e + d;
Console.WriteLine(@"Euro + Dólar (€18,09xx): {0}", aux_e.GetAmount());
aux_e = e + p;
Console.WriteLine("Euro + Pesos (€10,40xx): {0}", aux_e.GetAmount());
Console.WriteLine("----------------------------------------------");

Dolar aux_d = d + e;
Console.WriteLine("Dólar + Euro (U$S22,36xx): {0}", aux_d.GetAmount());
aux_d = d + p;
Console.WriteLine("Dólar + Pesos (U$S10,49xx): {0}", aux_d.GetAmount());
Console.WriteLine("----------------------------------------------");

Peso aux_p = p + e;
Console.WriteLine("Pesos + Euro ($259,26xx): {0}", aux_p.GetAmount());
aux_p = p + d;
Console.WriteLine("Pesos + Dólar ($211,65xx): {0}", aux_p.GetAmount());